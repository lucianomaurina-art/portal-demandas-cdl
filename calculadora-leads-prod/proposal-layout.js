...
window.generateProposal=async function(){
  ...
  let requestNotes='',requestCnaes='',requestProposalNotes='';
  if(currentRequestId){
    const {data:req}=await sb.from('lead_quote_requests').select('notes,client').eq('id',currentRequestId).maybeSingle();
    requestNotes=req?.notes||'';
    requestCnaes=req?.client?.cnaes_segments||req?.client?.cnae_segments||req?.client?.target_segments||'';
    requestProposalNotes=req?.client?.proposal_notes||'';
  }
  ...
  const c=clientData(),cdlLogo=new URL('../logo-cdl.svg',location.href).href,spcLogo='https://mvl-aces.nyc3.digitaloceanspaces.com/upload/produtosservicos/g_foto399.jpg';
  const issuedAt=new Date(),validUntil=new Date(issuedAt);validUntil.setDate(validUntil.getDate()+7);
  ...
  const rows=(quote.items||[]).map(i=>{const details=Array.isArray(i.details)?i.details:[];const detailHtml=details.length?`<div class="included"><b>Inclui:</b><ul>${details.map(d=>`<li>${h(d)}</li>`).join('')}</ul></div>`:`<div class="included single">${mode==='individual'?'Dado selecionado para aquisição.':'Item adicional contratado.'}</div>`;return `<tr><td><b>${h(i.name)}</b>${detailHtml}</td><td class="price">${money(i.commercial_unit)} / lead</td></tr>`}).join('');
  const notesHtml=requestNotes?`<h2 class="section-title">Observações da solicitação</h2><div class="notes-box">${h(requestNotes)}</div>`:'';
  const cnaeText=requestCnaes||c?.cnaes_segments||c?.cnae_segments||'';
  const cnaeHtml=cnaeText?`<h2 class="section-title">CNAEs ou segmentos desejados</h2><div class="target-box">${h(cnaeText)}</div>`:'';
  const proposalNotesHtml=requestProposalNotes?`<h2 class="section-title">Informações adicionais da proposta</h2><div class="proposal-notes-box">${h(requestProposalNotes)}</div>`:'';

  w.document.open();
  w.document.write(`<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>${h(saved.code)}</title><style>
  *{box-sizing:border-box}body{font-family:Arial,Helvetica,sans-serif;padding:42px;color:#142033;max-width:920px;margin:auto;background:#fff}.brandbar{display:flex;align-items:center;justify-content:space-between;gap:30px;border-bottom:3px solid #0b62d6;padding-bottom:20px;margin-bottom:24px}.brands{display:flex;align-items:center;gap:28px;min-height:72px}.brand-cdl{width:155px;max-height:70px;object-fit:contain}.brand-spc{width:170px;max-height:72px;object-fit:contain}.proposal-id{text-align:right;color:#667085;font-size:13px}.proposal-id strong{display:block;color:#071b33;font-size:16px;margin-bottom:4px}h1{color:#071b33;font-size:30px;margin:0 0 8px}.subtitle{color:#667085;margin:0 0 20px}.issuebox{border:2px solid #0b62d6;border-radius:14px;padding:16px 18px;margin:0 0 22px;background:#f4f8ff}.issue-title{font-size:11px;font-weight:900;letter-spacing:.1em;text-transform:uppercase;color:#0b62d6;margin-bottom:10px}.issuegrid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px 24px;font-size:13px}.issuegrid b{color:#071b33}.client-box{background:#f4f7fb;border:1px solid #dce3ed;border-radius:14px;padding:18px 20px;margin-bottom:24px}.client-box h3{margin:0 0 8px;color:#071b33}.meta{line-height:1.65;color:#344054}.section-title{font-size:18px;color:#071b33;margin:28px 0 10px}.notes-box{background:#fffaf0;border:1px solid #efd39a;border-left:4px solid #d59a27;border-radius:12px;padding:16px 18px;line-height:1.6;color:#344054;white-space:pre-wrap}.target-box{background:#eef6ff;border:1px solid #bfd4ee;border-left:4px solid #0b62d6;border-radius:12px;padding:16px 18px;line-height:1.6;color:#344054;white-space:pre-wrap}.proposal-notes-box{background:#f5f9ff;border:1px solid #bfd4ee;border-left:4px solid #0b62d6;border-radius:12px;padding:16px 18px;line-height:1.6;color:#344054;white-space:pre-wrap}.terms{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-top:10px}.term{border:1px solid #dce3ed;background:#f8fbff;border-radius:12px;padding:14px;line-height:1.45;font-size:13px}.term b{display:block;color:#071b33;margin-bottom:5px}.lgpd{margin-top:10px;border:1px solid #bfd4ee;border-left:4px solid #0b62d6;background:#f5f9ff;border-radius:12px;padding:17px 18px;color:#344054;font-size:12.5px;line-height:1.6}.lgpd b{color:#071b33}.lgpd ul{margin:9px 0 0;padding-left:19px}.lgpd li{margin:5px 0}table{width:100%;border-collapse:collapse;border:1px solid #dce3ed}th{background:#f4f7fb;color:#344054;font-size:13px}td,th{padding:12px 14px;border-bottom:1px solid #e7ecf2;text-align:left;vertical-align:top}.price{width:190px;white-space:nowrap;font-weight:700}.included{margin-top:8px;color:#475467;font-size:12px;line-height:1.5}.included ul{margin:5px 0 0;padding-left:18px;columns:2}.included.single{font-style:italic;color:#667085}.total-box{margin-top:24px;background:#071b33;color:#fff;border-radius:16px;padding:22px}.total-label{font-size:13px;opacity:.8;text-transform:uppercase}.total{font-size:32px;font-weight:800;margin-top:5px}.validity{margin-top:12px;font-size:13px}.cta{margin-top:26px;padding:18px 20px;background:#eef6ff;border-left:4px solid #0b62d6;border-radius:10px;line-height:1.55}.footer{margin-top:34px;padding-top:18px;border-top:1px solid #dce3ed;color:#667085;font-size:12px;display:flex;justify-content:space-between}.print{margin-top:24px;padding:11px 16px;border:0;border-radius:10px;background:#0b62d6;color:#fff;font-weight:700;cursor:pointer}@media(max-width:700px){.terms{grid-template-columns:1fr}}@media print{body{padding:20px}.print{display:none}.issuebox,.total-box,.notes-box,.target-box,.proposal-notes-box,.terms,.lgpd{break-inside:avoid}.total-box{print-color-adjust:exact;-webkit-print-color-adjust:exact}}
  </style></head><body>
  ...
  <div class="issuebox"><div class="issue-title">Informações da proposta</div><div class="issuegrid"><div><b>Data de emissão:</b> ${h(fmtDate(issuedAt))}</div><div><b>Horário de emissão:</b> ${h(fmtTime(issuedAt))}</div><div><b>Validade:</b> 7 dias</div><div><b>Válida até:</b> ${h(fmtDate(validUntil))}</div><div><b>Emissor:</b> ${h(issuerName)}</div><div><b>Login:</b> ${h(issuerEmail)}</div></div></div>
  ...
  <h2 class="section-title">Condições de entrega e pagamento</h2><div class="terms"><div class="term"><b>Prazo médio de entrega</b>Os leads serão enviados, em média, em até 15 dias após a contratação.</div><div class="term"><b>Forma de pagamento</b>O pagamento será realizado após o recebimento dos leads pelo cliente.</div><div class="term"><b>Formato de entrega</b>A base será entregue em planilha, nos formatos Google Planilhas ou Microsoft Excel.</div></div>
  <h2 class="section-title">Uso e proteção dos dados — LGPD</h2><div class="lgpd"><b>Ao contratar e receber a base de dados, o cliente declara ciência e concordância de que os dados deverão ser utilizados exclusivamente no âmbito de sua própria empresa, para finalidades comerciais legítimas relacionadas às suas atividades, observando a legislação aplicável de proteção de dados pessoais, especialmente a Lei Geral de Proteção de Dados Pessoais (LGPD).</b><ul><li>Os dados não poderão ser vendidos, cedidos, compartilhados, transferidos ou disponibilizados a terceiros fora da organização contratante, salvo quando houver base legal e autorização contratual aplicável.</li><li>É vedada a publicação, exposição ou disponibilização pública da base, integral ou parcialmente, inclusive em sites, redes sociais, repositórios públicos ou outros meios de acesso indiscriminado.</li><li>O cliente é responsável por adotar medidas técnicas e administrativas adequadas para proteger os dados contra acessos não autorizados, perda, vazamento, alteração ou uso indevido.</li><li>O uso dos dados deverá respeitar a finalidade para a qual foram contratados e as demais obrigações aplicáveis previstas na LGPD.</li></ul></div>
