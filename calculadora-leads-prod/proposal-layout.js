...
window.generateProposal=async function(){
  ...
  let requestNotes='';
  if(currentRequestId){const {data:req}=await sb.from('lead_quote_requests').select('notes').eq('id',currentRequestId).maybeSingle();requestNotes=req?.notes||'';}
