// Informações adicionais da solicitação — uso interno.
// Mantém a observação original do cliente intacta e salva complementos em client.internal_notes.
(() => {
  ...
  function ensureRequestDialogLocal(){
    ...
    d.style.cssText='width:min(800px,calc(100% - 30px));border:0;border-radius:18px;padding:0;box-shadow:0 25px 80px rgba(0,0,0,.28)';
  ...
  window.saveRequestInternalNotes=async function(id){
    const input=document.getElementById('requestInternalNotesInput');
    const btn=document.getElementById('saveRequestInternalNotesBtn');
    const msg=document.getElementById('requestInternalNotesMsg');
    if(!input)return;
    ...
    try{
      ...
      const client={...(r?.client||{}),internal_notes:input.value.trim()};
      const {error}=await sb.from('lead_quote_requests').update({client,updated_at:new Date().toISOString()}).eq('id',id);
      if(error)throw error;
      if(msg){msg.textContent='Informações adicionais salvas.';msg.style.color='#176b4d';}
      if(btn)btn.textContent='Salvo';
      setTimeout(()=>{if(btn){btn.disabled=false;btn.textContent='Salvar informações adicionais';}},900);
    }catch(error){
      console.error(error);
      if(msg){msg.textContent='Não foi possível salvar as informações adicionais.';msg.style.color='#b42318';}
      if(btn){btn.disabled=false;btn.textContent='Salvar informações adicionais';}
    }
  };
  ...
  window.viewRequest=async function(id){
    ...
    d.innerHTML=`<div style="padding:24px">
      ...
      <div style="margin-top:20px;padding:16px;background:#fff8e8;border:1px solid #efd39a;border-radius:12px"><div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start"><div><b>Informações adicionais / observações internas</b><div class="muted" style="font-size:12px;margin-top:4px">Use este espaço para registrar informações obtidas após a solicitação. Este conteúdo é interno e não altera o texto original enviado pelo cliente.</div></div><span style="font-size:10px;font-weight:900;color:#8a5a00;background:#fff1d6;padding:5px 8px;border-radius:999px">USO INTERNO</span></div><textarea id="requestInternalNotesInput" rows="5" style="width:100%;margin-top:12px;border:1px solid #dce3ed;border-radius:10px;padding:12px;resize:vertical" placeholder="Ex.: após contato com o cliente, confirmou que deseja empresas do setor industrial, com faturamento...">${escReq(internalNotes)}</textarea><div id="requestInternalNotesMsg" class="muted" style="font-size:12px;margin-top:7px"></div><div style="display:flex;justify-content:flex-end;margin-top:10px"><button id="saveRequestInternalNotesBtn" class="secondary" onclick="saveRequestInternalNotes('${r.id}')">Salvar informações adicionais</button></div></div>
