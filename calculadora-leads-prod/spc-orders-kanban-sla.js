...
(()=>{
  const STAGES=[
    ...
    {key:'Dados enviados ao cliente',label:'Dados enviados ao cliente',sla:'Entrega concluída'},
    {key:'Pós-venda',label:'Pós-venda',sla:'Contato 60 dias após o envio'}
  ];
  ...
  function effectiveStage(o){return o.filters?.post_sale===true?'Pós-venda':o.stage}
  function stageDue(o){
    const current=effectiveStage(o);
    if(o.stage_due_at)return date(o.stage_due_at);
    if(current==='Solicitar contagem')return addDays(o.proposal_closed_at||o.proposal_updated_at||o.created_at,1);
    if(current==='Contagem enviada ao SPC')return addDays(o.count_sent_at||o.count_requested_at||o.updated_at,7);
    if(current==='Validar contagem')return addDays(o.count_returned_at||o.updated_at,1);
    if(current==='Aguardando planilha de dados')return addDays(o.production_requested_at||o.count_validated_at||o.updated_at,6);
    if(current==='Pós-venda')return addDays(o.data_delivered_at,60);
    return null;
  }
  function overallDue(o){return date(o.overall_due_at)||addDays(o.proposal_closed_at||o.proposal_updated_at||o.created_at,15)}
  function state(o){
    if(effectiveStage(o)==='Dados enviados ao cliente')return 'done';
  ...
  function overallState(o){
    ...
    if(['Dados enviados ao cliente','Pós-venda'].includes(effectiveStage(o)))return o.data_delivered_at&&date(o.data_delivered_at)>due?'late':'done';
    const ms=due-Date.now();if(ms<0)return 'late';if(ms<=86400000)return 'warn';return 'ok';
  }
  ...
  async function fetchPending(orders){
    const {data:allOrders}=await sb.from('spc_data_orders').select('proposal_id');
    const used=new Set((allOrders||orders).map(o=>o.proposal_id));
  ...
  function cardHtml(o,isAdmin){
    const c=o.client||{},current=effectiveStage(o),st=state(o),ost=overallState(o),due=stageDue(o),total=overallDue(o),info=stageInfo(current);
    const opts=STAGES.map(x=>`<option value="${esc(x.key)}" ${x.key===current?'selected':''}>${esc(x.label)}</option>`).join('');
    const ticket=o.filters?.spc_ticket_number||'';
    return `<article class="spc-sla-card ${st}" draggable="true" data-order="${o.id}"><div class="spc-sla-code">${esc(o.proposal_code||c.proposal_code||'ORDEM SPC')}</div><h4>${esc(c.company||'Cliente')}</h4><div class="spc-sla-meta">${esc(o.product||'SPC Dados')} • ${esc(o.person||'')}<br>${qty(c.quantity)} leads${ticket?`<br><b>Chamado SPC:</b> ${esc(ticket)}`:''}${o.count_result!=null?` • Contagem: ${qty(o.count_result)}`:''}</div>${due?`<div class="spc-sla-timer">${current==='Pós-venda'?'Pós-venda • ':esc(colorLabel(st))+' • '}${esc(remaining(due))}<br><span style="font-weight:600">${current==='Pós-venda'?'Contato programado':'Prazo da etapa'}: ${esc(fmt(due))}</span></div>`:''}<div class="spc-sla-total">Entrega ao cliente até <strong class="${ost}">${esc(fmt(total))}</strong> • SLA total 15 dias</div><div class="spc-sla-actions"><button class="secondary" data-open="${o.id}">Abrir ordem</button><select data-stage="${o.id}" aria-label="Mover etapa">${opts}</select>${isAdmin?`<button class="danger" onclick="inactivateSpcOrder('${o.id}')">Inativar</button>`:''}</div></article>`;
  }
  ...
  async function render(){
    ...
    injectStyles();
    if(typeof loadCurrentRole==='function')await loadCurrentRole();const adminUser=typeof isAdmin==='function'&&isAdmin(),orders=await fetchOrders(),pending=await fetchPending(orders);
  ...
  async function moveStage(id,next){
    ...
    if(effectiveStage(o)===next)return;
    if(STAGES.findIndex(x=>x.key===next)>=STAGES.findIndex(x=>x.key==='Contagem enviada ao SPC')&&(!String(o.filters?.spc_ticket_number||'').trim()||!o.count_sent_at)){
      alert('Para registrar o envio ao SPC, informe o número do chamado/protocolo e a data de envio.');
      return window.prepareSpcOrderStage?window.prepareSpcOrderStage(id,next):window.editSpcOrder?.(id);
    }
    ...
    const filters={...(o.filters||{}),post_sale:next==='Pós-venda',post_sale_started_at:next==='Pós-venda'?(o.filters?.post_sale_started_at||new Date().toISOString()):null},payload=next==='Pós-venda'?{stage:'Dados enviados ao cliente',filters}:{stage:next,filters};
    const {error:e}=await sb.from('spc_data_orders').update(payload).eq('id',id);
